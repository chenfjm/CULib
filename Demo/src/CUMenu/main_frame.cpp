...
bool MainFrame::OnHidden(void* param)
{
	TNotifyUI* pMsg = (TNotifyUI*)param;
	if( pMsg->sType == _T("itemactivate") ) 
	{
		CButtonUI* pButton=(CButtonUI*)(paint_manager_.FindControl(_T("menuBtn")));
		if(pButton)
...
void MainFrame::Notify(TNotifyUI& msg)
{
	...
	else if (_tcsicmp(msg.sType, kClick) == 0)
	{
		...
		else if (_tcsicmp(msg.pSender->GetName(), _T("menuBtn")) == 0)	
		{
			CMenuWnd* pMenu = new CMenuWnd(m_hWnd);
			RECT rect;
			rect=paint_manager_.GetCaptionRect();
			CPoint point(rect.left,rect.bottom);
			ClientToScreen(m_hWnd, &point);
			STRINGorID xml(IDR_XML_MENU);
			pMenu->Init(NULL, xml, _T("xml"), point);
			CMenuElementUI* pElement=(CMenuElementUI*)pMenu->m_pm.FindControl(_T("hidden"));
			if(pElement)
				pElement->OnNotify += MakeDelegate(this,&MainFrame::OnHidden);
		}
	}
