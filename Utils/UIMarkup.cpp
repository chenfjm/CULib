...
bool CMarkup::LoadFromFile(LPCTSTR pstrFilename, int encoding)
{
	...
	CStdString sLog=sFile;
	sLog+=_T("UILog.txt");
	GELog* pLog=GELog::GetSingleton();
	pLog->CreateLog(sLog);
	//GELog::GetSingleton()->AppendLog(_T("sdfsfsdfsdfs"));
	pLog->AppendLog(_T("我支持中文"));
	pLog->AppendLog(_T("%d"),8888);
	GELog::GetSingleton()->AppendLog(_T("%d"),8888);
    if( CPaintManagerUI::GetResourceZip().IsEmpty() ) {
        sFile += pstrFilename;
        HANDLE hFile = ::CreateFile(sFile, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
        if( hFile == INVALID_HANDLE_VALUE ) 
		{
			pLog->AppendLog(_T("xml文件打开失败"));
			return _Failed(_T("Error opening file"));
		}
        DWORD dwSize = ::GetFileSize(hFile, NULL);
        if( dwSize == 0 )
		{
			pLog->AppendLog(_T("xml文件为空"));
			return _Failed(_T("File is empty"));
		}
        if ( dwSize > 4096*1024 )
		{
			pLog->AppendLog(_T("xml文件太大"));
		...
		{
			pLog->AppendLog(_T("zip文件中未找到对应项"));
...
bool CMarkup::_Parse(LPTSTR& pstrText, ULONG iParent)
{
    ...
    for( ; ; ) 
    {
        ...
        if( *pstrText != _T('<') ) 
		{
			GELog::GetSingleton()->AppendLog(_T("Node:%d  标签意外开始"),iPrevious);
        ...
        *pstrText++ = _T('\0');
		...
		CStdString temp;
        temp=_SkipIdentifier(pstrText);
        LPTSTR pstrNameEnd = pstrText;
        if( *pstrText == _T('\0') )
		{
			GELog::GetSingleton()->AppendLog(_T("Node:%d Name:%s  标签意外开始"),iPrevious,temp);
			return _Failed(_T("Error parsing element name"), pstrText);
		}
        // Parse attributes
        if( !_ParseAttributes(pstrText,iPos) ) return false;
        _SkipWhitespace(pstrText);
        ...
        else
        {
            if( *pstrText != _T('>') )
			{
				GELog::GetSingleton()->AppendLog(_T("Node:%d  标签意外结束"),iPos);
            ...
            if( *pstrText != _T('<') ) 
			{
				GELog::GetSingleton()->AppendLog(_T("Node:%d  标签意外开始"),iPos);
            ...
            if( pstrText[0] == _T('<') && pstrText[1] == _T('/') ) 
            {
                ...
                *pstrText = _T('\0');
				...
				{
					GELog::GetSingleton()->AppendLog(_T("Node:%d  不匹配的结束标签"),iPos);
...
CStdString CMarkup::_SkipIdentifier(LPTSTR& pstr) const
{
    // 属性只能用英文，所以这样处理没有问题
	CStdString pname;
...
bool CMarkup::_ParseAttributes(LPTSTR& pstrText,int ipos)
{   
	CStdString patname;
    if( *pstrText == _T('>') ) 
	{
		GELog::GetSingleton()->AppendLog(_T("Node:%d  空标签"),ipos);
		return true;
	}
    *pstrText++ = _T('\0');
    _SkipWhitespace(pstrText);
    while( *pstrText != _T('\0') && *pstrText != _T('>') && *pstrText != _T('/') ) {
		patname.Empty();
        patname=_SkipIdentifier(pstrText);
        LPTSTR pstrIdentifierEnd = pstrText;

        _SkipWhitespace(pstrText);
        if( *pstrText != _T('=') )
		{
			GELog::GetSingleton()->AppendLog(_T("Node:%d  属性名%s后缺少="),ipos,patname);
        ...
        *pstrIdentifierEnd = _T('\0');
        ...
        if( *pstrText++ != _T('\"') ) 
		{
			GELog::GetSingleton()->AppendLog(_T("Node:%d  属性名%s后缺少引号"),ipos,patname);
        ...
        if( *pstrText == _T('\0') ) 
		{
			GELog::GetSingleton()->AppendLog(_T("Node:%d  属性名%s意外结束"),ipos,patname);
