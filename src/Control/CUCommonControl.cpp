...
LPCTSTR CPictureBox::GetClass() const
{
		return _T("PictureBox");
}
LPVOID CPictureBox::GetInterface(LPCTSTR pstrName)
{
		if( _tcscmp(pstrName, _T("PictureBox")) == 0 ) return static_cast<CPictureBox*>(this);
		return CControlUI::GetInterface(pstrName);
}

CLabelUI::CLabelUI() : m_uTextStyle(DT_VCENTER), m_dwTextColor(0), 
	m_dwDisabledTextColor(0), m_iFont(-1), m_bShowHtml(false),m_hwndTooltip(NULL)
{
...
void CButtonUI::DoEvent(TEventUI& event)
{
	...
	if( event.Type == UIEVENT_MOUSEENTER )
	{
		m_pManager->SetTimer(this,MOUSEMOVE_TIMERID,500);
		if( IsEnabled() ) {
