...
typedef struct tagFINDDIRECTINFO
{
	CControlUI* pFocus;//焦点控件
	CControlUI* pCurrent;//最近控件
...
CStdPtrArray CPaintManagerUI::m_aNotifiers;
...
case WM_MOUSEMOVE:
	...
	// Start tracking this entire window again...
		...
		GetCursorPos(&pe);
		m_ptLastMousePos = pt;
		if( m_bMoveFlag && wParam == MK_LBUTTON)
		{
			tmp.x=m_swPosition.left+pe.x-m_ptLastScreenMouse.x;
			tmp.y=m_swPosition.top+pe.y-m_ptLastScreenMouse.y;
			if(!MoveWindow(m_hWndPaint,tmp.x,tmp.y,(int)(m_swPosition.right-m_swPosition.left)\
						,(int)(m_swPosition.bottom-m_swPosition.top),false))
						m_pRoot->Invalidate();
			GetWindowRect(m_hWndPaint,&m_swPosition);
			m_ptLastScreenMouse=pe;
			return true;
		}
		...
		else if( pNewHover != m_pEventHover && pNewHover != NULL ) {
...
case WM_LBUTTONDOWN:
	{
		...
		POINT pt = { GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam) };
		m_ptLastMousePos = pt;
		GetCursorPos(&m_ptLastScreenMouse);
...
case WM_KEYDOWN:
	{
		...
		//if(wParam==VK_MENU)
		///{
		//	::SendMessage(m_hWndPaint,WM_CONTEXTMENU,wParam,lParam);
		//	break;
		//}
		TEventUI event = { 0 };
...
bool CPaintManagerUI::AttachDialog(CControlUI* pControl)
{
	...
	m_bUpdateNeeded = true;
	m_bFirstLayout = true;
	/** m_bFocusNeeded初始化必须为false，否则会引起图片不能正常显示 */
...
{
	// Translates Message takes care of system-wide messages, such as
...
bool CPaintManagerUI::SetLeftControl()
{
	FINDDIRECTINFO info;
	info.pFocus=m_pFocus;
	if (m_pFocus->IsLeft())
	{
		CControlUI* pControl=m_pRoot->FindControl(__FindControlsFromLeft,&info,UIFIND_VISIBLE | UIFIND_ENABLED | UIFIND_ME_FIRST);
		pControl=info.pCurrent;
		if (pControl!=NULL)
		{
			SetFocus(pControl);//
		}
...
bool CPaintManagerUI::SetRightControl()
{
	FINDDIRECTINFO info;
	info.pFocus=m_pFocus;
	if (m_pFocus->IsRight())
	{
		CControlUI* pControl=m_pRoot->FindControl(__FindControlsFromRight,&info,UIFIND_VISIBLE | UIFIND_ENABLED | UIFIND_ME_FIRST);
		pControl=info.pCurrent;
		if (pControl!=NULL)
		{
			SetFocus(pControl);
...
bool CPaintManagerUI::SetUpControl()
{
	FINDDIRECTINFO info;
	info.pFocus=m_pFocus;
	if (m_pFocus->IsUp())
	{
		CControlUI* pControl=m_pRoot->FindControl(__FindControlsFromUp,&info,UIFIND_VISIBLE | UIFIND_ENABLED | UIFIND_ME_FIRST);
		pControl=info.pCurrent;
		if (pControl!=NULL)
		{
			SetFocus(pControl);
...
bool CPaintManagerUI::SetDownControl()
{
	FINDDIRECTINFO info;
	info.pFocus=m_pFocus;
	if (m_pFocus->IsDown())
	{
		CControlUI* pControl=m_pRoot->FindControl(__FindControlsFromDown,&info,UIFIND_VISIBLE | UIFIND_ENABLED | UIFIND_ME_FIRST);
		pControl=info.pCurrent;
		if (pControl!=NULL)
		{
			SetFocus(pControl);
...
CControlUI* CALLBACK CPaintManagerUI::__FindControlsFromLeft(CControlUI* pThis, LPVOID pData)//向左找
{
	FINDDIRECTINFO* pdirect=static_cast<FINDDIRECTINFO*>(pData);
	RECT focus=pdirect->pFocus->GetPos();
	RECT current=pThis->GetPos();

	if (pdirect->pFocus==pThis)
	{
		return NULL;
	}

	if (pThis->GetControlFlags()==0)	return NULL;//屏蔽容器、布局
	if (!((current.top-focus.bottom)<0&&(current.bottom-focus.top)>0))	//屏蔽不在同一水平线
	{
		return NULL;
	}
	if ((focus.left-current.left)<=0)//屏蔽右边的
	{
		return NULL;
	}
	if((focus.left-current.right)<pdirect->distance)
	{
		pdirect->distance=(focus.left-current.right);//更新距离
		pdirect->pCurrent=pThis;		//记录控件
	}
...
CControlUI* CALLBACK CPaintManagerUI::__FindControlsFromRight(CControlUI* pThis, LPVOID pData)
{
	FINDDIRECTINFO* pdirect=static_cast<FINDDIRECTINFO*>(pData);
	RECT focus=pdirect->pFocus->GetPos();
	RECT current=pThis->GetPos();
	...
	if (pThis->GetControlFlags()==0)	return NULL;//屏蔽容器、布局
	//if ((focus.top-current.bottom)>0||(current.top-focus.bottom)>0)	//屏蔽不在同一水平线
	//{
	//		return NULL;
	//}
	if (!((current.top-focus.bottom)<0&&(current.bottom-focus.top)>0))	//屏蔽不在同一水平线
	...
	if ((focus.right-current.right)>=0)//屏蔽左边的
	{
		return NULL;
	}
	if((current.left-focus.right)<pdirect->distance)
	{
		pdirect->distance=(current.left-focus.right);//更新距离
		pdirect->pCurrent=pThis;		//记录控件
	}
...
CControlUI* CALLBACK CPaintManagerUI::__FindControlsFromUp(CControlUI* pThis, LPVOID pData)//向上找
{
	FINDDIRECTINFO* pdirect=static_cast<FINDDIRECTINFO*>(pData);
	RECT focus=pdirect->pFocus->GetPos();
	RECT current=pThis->GetPos();
	...
	if (pThis->GetControlFlags()==0)	return NULL;//屏蔽容器、布局
	if ((current.top-focus.bottom)<0&&(current.bottom-focus.top)>0)	//屏蔽同一水平线
	{
		return NULL;
	}
	if ((focus.top-current.top)<=0)//屏蔽下边的
	{
		return NULL;
	}
	if((focus.top-current.bottom)<pdirect->distance)
	{
		pdirect->distance=(focus.top-current.bottom);//更新距离
		pdirect->pCurrent=pThis;		//记录控件
	}
...
CControlUI* CALLBACK CPaintManagerUI::__FindControlsFromDown(CControlUI* pThis, LPVOID pData)//向下找
{
	...
	if (pThis->GetControlFlags()==0)	return NULL;//屏蔽容器、布局
	//if ((focus.top-current.bottom)>0||(current.top-focus.bottom)<0)	//屏蔽同一水平线
	//{
	//	return NULL;
	//}
	if ((current.top-focus.bottom)<0&&(current.bottom-focus.top)>0)	//屏蔽同一水平线
	...
	if (focus.bottom-current.bottom>=0)//屏蔽上边的
	{
		return NULL;
	}
	if((current.top-focus.bottom)<pdirect->distance)
	{
		pdirect->distance=(current.top-focus.bottom);//更新距离
		pdirect->pCurrent=pThis;		//记录控件
	}
