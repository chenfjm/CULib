...
protected:
	 ...
	 *  @brief 鼠标激活 
	 *  
	 *  获取当前对象是否支持激活状态，如果支持则DoVerb(OLEIVERB_INPLACEACTIVATE)
	 */
...
/** 一个ActiveX对象的容器 */
class CActiveXCtrl :
	/** 由容器向嵌入对象提供位置 显示 名称 用户接口等资源 */
	public IOleClientSite,
	/** 使一个无窗口对象能够响应windows消息，参与拖拽操作 */
	public IOleInPlaceSiteWindowless,
	/** 提供使site对象能够管理容器中的嵌入控件的方法 */
	public IOleControlSite,
	/** 提供对象和在容器中的site的通信方式 */
	public IObjectWithSite,
	/** 枚举一个复合文档中的对象或锁处于运行状态的容器 */
...
bool CActiveXUI::DoCreateControl()
{
	...
	if( !m_sModuleName.IsEmpty() ) {
		...
		if( hModule != NULL ) {
			...
			DllGetClassObjectFunc aDllGetClassObjectFunc = (DllGetClassObjectFunc)::GetProcAddress(hModule, _T("DllGetClassObject"));
