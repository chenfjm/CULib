...
bool CListUI::Add(CControlUI* pControl,CControlUI* pParent)
{
...
void CTreeItemUI::DoEvent(TEventUI& event)
{
	...
	if( event.Type == UIEVENT_BUTTONDOWN || event.Type == UIEVENT_RBUTTONDOWN )
	{
		if( IsEnabled() ) {
			...
			//Select();
			//Invalidate();
			if( m_root->GetItemIndex(this) != -1 )
			{

				CTreeItemUI* node =(CTreeItemUI*)this->GetTag();

				POINT pt = { 0 };
				::GetCursorPos(&pt);
				::ScreenToClient(m_pManager->GetPaintWindow(), &pt);
				pt.x -= this->GetX();
				SIZE sz =m_root->GetExpanderSizeX(node);
				if( pt.x >= sz.cx && pt.x < sz.cy )                     
					m_root->SetChildVisible(node, !node->data()._child_visible);
	...
	if (event.Type==UIEVENT_DBLCLICK)
	{
		if( m_root->GetItemIndex(this) != -1 )
		{
			if( _tcscmp(this->GetClass(), _T("ListLabelElementUI")) == 0 ) {
				CTreeItemUI* node = (CTreeItemUI*)this->GetTag();
				m_root->SetChildVisible(node, !node->data()._child_visible);
...
///////////////////////////////////////////////////////////////
CControlUI* CTreeItemUI::GetItemAt(int iIndex) const
{
	return NULL;
}
int CTreeItemUI::GetItemIndex(CControlUI* pControl) const
{
	return 0;
}
bool CTreeItemUI::SetItemIndex(CControlUI* pControl, int iIndex)
{
	return true;
}
int CTreeItemUI::GetCount() const
{
	return 0;
}

void CTreeItemUI::SetHtmlText(LPCTSTR pstrHtml)
{
	data()._text=pstrHtml;
	CStdString html_text;
	html_text += _T("<x 6>");
	for( int i = 0; i <data()._level; ++i )
	{
		html_text += _T("<x 24>");
	}
	if(data()._level < 3 ) {
		if(data()._child_visible ) html_text +=data()._tree1;
		else html_text +=data()._tree2;
	}
	html_text +=data()._text;
	SetText(html_text);
	
	if(data()._level == 0 ) {
		SetBkImage(_T("file='tree_top.png' corner='2,1,2,1' fade='100'"));
	}
}

void CTreeItemUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	if( _tcscmp(pstrName, _T("htmltext")) == 0 ) SetHtmlText(pstrValue);
	else if(_tcscmp(pstrName, _T("tree1")) == 0 ) data()._tree1=pstrValue;
	else if(_tcscmp(pstrName, _T("tree2")) == 0 ) data()._tree2=pstrValue;
	else CListLabelElementUI::SetAttribute(pstrName, pstrValue);
}

bool CTreeItemUI::Add(CControlUI* pControl,CControlUI* pParent)
{
	CTreeItemUI* pItem=static_cast<CTreeItemUI*>(pParent->GetInterface(_T("TreeItem")));
	m_root->AddNode(pControl,pItem);
	return true;
}
...
bool CTreeItemUI::Remove(CControlUI* pControl)
{
	return true;
}
bool CTreeItemUI::RemoveAt(int iIndex)
{
	return true;
}
...
CTreeItemUI::CTreeItemUI() : _parent (NULL) 
{

}

CTreeItemUI::CTreeItemUI(NodeData t) : _data (t), _parent (NULL)
{

}

CTreeItemUI::CTreeItemUI(NodeData t, CTreeItemUI* parent)	: _data (t), _parent (parent) 
{
...
LPVOID CTreeItemUI::GetInterface(LPCTSTR pstrName)
{
	...
	else if( _tcscmp(pstrName, _T("TreeItem")) == 0 ) return static_cast<CTreeItemUI*>(this);
	return CListLabelElementUI::GetInterface(pstrName);
}
void CTreeItemUI::add_child(CTreeItemUI* child) 
{
	child->set_parent(this); 
	_children.push_back(child); 
}

void CTreeItemUI::remove_child(CTreeItemUI* child)
{
...
CTreeItemUI* CTreeItemUI::get_last_child()
{
	if( has_children() )
	{
...
CTreeViewUI::CTreeViewUI() : _root(NULL), m_dwDelayDeltaY(0), m_dwDelayNum(0), m_dwDelayLeft(0)
{
	SetItemShowHtml(true);

	_root = new CTreeItemUI;
	_root->data()._level = -1;
	_root->data()._child_visible = true;
//	_root->data()._pListElement = NULL;
}

bool CTreeViewUI::Add(CControlUI* pControl,CControlUI* pParent)
{
	AddNode(pControl);
	return true;
}
...
bool CTreeViewUI::Remove(CControlUI* pControl)
{
	if( !pControl ) return false;
	if( _tcscmp(pControl->GetClass(), _T("ListLabelElementUI")) == 0 ) return false;
...
CTreeItemUI* CTreeViewUI::AddNode(CControlUI* pControl,CTreeItemUI* parent)
{
	if( !parent ) parent = _root;


	CTreeItemUI* node=static_cast<CTreeItemUI*>(pControl->GetInterface(_T("TreeItem")));
	...
	if( parent != _root && !parent->IsVisible() )
	{
		node->SetVisible(false);
	}
	...
	node->SetText(html_text);

	
	if( node->data()._level == 0 ) {
		node->SetBkImage(_T("file='tree_top.png' corner='2,1,2,1' fade='100'"));
	}*/

	node->SetTag((UINT_PTR)node);

	int index = 0;
	if( parent->has_children() )
	{
		CTreeItemUI* prev = parent->get_last_child();
		index = prev->GetIndex() + 1;
	}
	else 
	{
		if( parent == _root ) index = 0;
		else index = parent->GetIndex() + 1;
	}
	if( !CListUI::AddAt(node, index) ) {
		delete node;
		node = NULL;
	}
...
void CTreeViewUI::RemoveNode(CTreeItemUI* node)
{
	if( !node || node == _root ) return;

	for( int i = 0; i < node->num_children(); ++i )
	{
		CTreeItemUI* child = node->child(i);
		RemoveNode(child);
	}
...
void CTreeViewUI::SetChildVisible(CTreeItemUI* node, bool visible)
{
	if( !node || node == _root ) return;
	...
	if( node->data()._level < 3 ) {
		if( node->data()._child_visible ) html_text +=node->data()._tree1;
		else html_text +=node->data()._tree2;
	}
	html_text += node->data()._text;
	node->SetText(html_text);
	...
	CTreeItemUI* begin = node->child(0);
	CTreeItemUI* end = node->get_last_child();
	for( int i = begin->GetIndex(); i <= end->GetIndex(); ++i )
	{
		...
		if( _tcscmp(control->GetClass(), _T("ListLabelElementUI")) == 0 )
		{
			...
			{
				CTreeItemUI* local_parent = ((CTreeItemUI*)control->GetTag())->parent();
				if( local_parent->data()._child_visible && local_parent->IsVisible() )
				{
...
SIZE CTreeViewUI::GetExpanderSizeX(CTreeItemUI* node) const
{
