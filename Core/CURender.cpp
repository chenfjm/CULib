...
* @param    HDC hDC 窗口句
* @param    RECT rc rc区域
* @param    CRenderClip & clip clip[out]剪切区
* @return
* @retval   void
*/
...
* @param    RECT rc 区域
* @param    RECT rcItem
* @param    int width 横向圆角弧
* @param    int height 纵向圆角弧
* @param    CRenderClip & clip [out]剪切区
* @return
* @retval   void
*/
...
* @brief	加载图像
* 
* @param    STRINGorID bitmap 图像
* @param    LPCTSTR type 类型
* @param    DWORD mask 隐蔽色
* @return	返回TImageInfo结构
* @retval   TImageInfo*
* 读取文件，资源和zip包中图像文件数据到内存
* 创建DIB
...
* @param    const RECT & rc 对象区域
* @param    const RECT & rcPaint 绘制区域
* @param    const RECT & rcBmpPart 位图区域
* @param    const RECT & rcCorners 拉伸区域
* @param    bool alphaChannel alpha通道
* @param    BYTE uFade 透明度
* @param    bool hole 居中
* @param    bool xtiled 横向平铺
* @param    bool ytiled 纵向平铺
* @return
* @retval   void
...
*
* @param    HDC hDC 设备句柄 
* @param    CPaintManagerUI * pManager 绘制管理者
* @param    const RECT & rc 对象区域
* @param    const RECT & rcPaint 绘制区域
* @param    LPCTSTR pStrImage 标识字符串
* @param    LPCTSTR pStrModify 标识字符串
* @return	成功返回true失败返回false
* @retval   bool
* 实现：
* 分析标识字符串获取属性
* 先分析pStrImage信息后再分析pStrModify的信息
* 获取图像数据
* 绘制图像
*/
		...
		while( *pStrImage != _T('\0') ) {
			...
			if( !sValue.IsEmpty() ) {
				if( sItem == _T("file") || sItem == _T("res") ) {
					if( image_count > 0 )
						CULib::DrawImage(hDC, pManager, rc, rcPaint, sImageName, sImageResType,
...
* @param    HDC hDC 设备句柄
* @param    const RECT & rc 对象区域
* @param    DWORD color 颜色(alpha, r, g, b)
* @return
* @retval   void
* 实现：
...
* @param    HDC hDC 设备句柄
* @param    const RECT & rc 对象区域
* @param    DWORD dwFirst 颜色1
* @param    DWORD dwSecond 颜色2
* @param    bool bVertical 渐变方向 true垂直 false水平
* @param    int nSteps 步长
* @return
* @retval   void
* 实现：
...
* @param    CPaintManagerUI * pManager 绘制管理者
* @param    RECT & rc 对象区域
* @param    LPCTSTR pstrText "mini-html" 标识字符串
* @param    DWORD dwTextColor 字符串类型
* @param    RECT * prcLinks 产生链接效果的区域组
* @param    CStdString * sLinks 产生链接效果的区域组数量
* @param    int & nLinkRects 产生链接效果的区域组数量
* @param    UINT uStyle 类型，同DrawText() Format Flags
* @return
* @retval   void
*/
void CRenderEngine::DrawHtmlText(HDC hDC, CPaintManagerUI* pManager, RECT& rc, LPCTSTR pstrText, DWORD dwTextColor, RECT* prcLinks, CStdString* sLinks, int& nLinkRects, UINT uStyle)
{
	...
	while( *pstrText != _T('\0') ) {
		...
		else if( !bInRaw &&  *pstrText == _T('<') && pstrText[2] == _T('>') && (pstrText[1] == _T('{')  || pstrText[1] == _T('}')) )
		{
			...
			if( bDraw && bLineDraw ) 
				ExtTextOut(hDC, pt.x, pt.y + cyLineHeight - pTm->tmHeight - pTm->tmExternalLeading, ETO_CLIPPED,NULL,&pstrText[1], 1,NULL);
			pt.x += szSpace.cx;
			cxMaxWidth = MAX(cxMaxWidth, pt.x);
		...
		else if( !bInRaw &&  *pstrText == _T(' ') )
		{
			...
			if( bDraw && bLineDraw ) 
				ExtTextOut(hDC, pt.x,  pt.y + cyLineHeight - pTm->tmHeight - pTm->tmExternalLeading,ETO_CLIPPED,NULL,  _T(" "), 1,NULL);
			pt.x += szSpace.cx;
			cxMaxWidth = MAX(cxMaxWidth, pt.x);
			pstrText++;
		}
		else
		{
			...
			::GetTextExtentPoint32(hDC, pstrText, cchSize, &szText);
			if( bDraw && bLineDraw ) {
				ExtTextOut(hDC, ptPos.x, ptPos.y + cyLineHeight - pTm->tmHeight - pTm->tmExternalLeading,ETO_CLIPPED,NULL, pstrText, cchSize,NULL);
				if( pt.x >= rc.right && (uStyle & DT_END_ELLIPSIS) != 0 ) 
					ExtTextOut(hDC, ptPos.x + szText.cx, ptPos.y,ETO_CLIPPED,NULL, _T("..."), 3,NULL);
			}
			pt.x += szText.cx;
			cxMaxWidth = MAX(cxMaxWidth, pt.x);
			pstrText += cchSize;
...
*
* @param    CPaintManagerUI * pManager 绘制管理者
* @param    CControlUI * pControl 控件句柄
* @param    RECT rc 对象区域
* @return	返回 BMP句柄
* @retval   HBITMAP
*/
