...
{
	if( _tcscmp(GetName(),_T("menuBtn"))==0)
		return true;
...
bool CControlUI::IsLeft()const
{
	return true;
}

bool CControlUI::IsRight()const
{
	return true;
}

bool CControlUI::IsUp()const
{
	return true;
}

bool CControlUI::IsDown()const
{
	return true;
}
...
* @brief	系统事件处理函数
*
* @param    TEventUI & event 界面库的消息类型，以UIEVENT_开头
* @retval   void
* CPaintManagerUI的MessageHandler函数把系统的Windows消息转换为界面库内部的消息
* 类型——TEventUI,并把该消息分发给相应控件。控件对事件消息的处理就在DoEvent函数
* 中。该函数通过CPAintManagerUI::SendNotify把事件消息通知给用户程序。用户程序在
* OnNotify函数中对事件进行处理。
*/
void CControlUI::DoEvent(TEventUI& event)
{
	...
	{
		m_pManager->SetTimer(this,MOUSEMOVE_TIMERID,1000);
		return;
	}
	if( event.Type == UIEVENT_MOUSELEAVE )
	{
		m_pManager->KillTimer(this,MOUSEMOVE_TIMERID);
		if( m_hwndTooltip != NULL ) ::SendMessage(m_hwndTooltip, TTM_TRACKACTIVATE, FALSE, (LPARAM) &m_ToolTip);
		if( m_bMouseTracking ) ::SendMessage(m_pManager->GetPaintWindow(), WM_MOUSEMOVE, 0, (LPARAM) -1);
	...
	if( event.Type == UIEVENT_TIMER )
	{
		POINT pe;
		GetCursorPos(&pe);
		ScreenToClient(m_pManager->GetPaintWindow(),&pe);
		CControlUI* pControl=m_pManager->FindControl(pe);
		if(pControl==this)
		{
			CStdString sToolTip = this->GetToolTip();
			if( sToolTip.IsEmpty() ) return ;
			::ZeroMemory(&m_ToolTip, sizeof(TOOLINFO));
			m_ToolTip.cbSize = sizeof(TOOLINFO);
			m_ToolTip.uFlags = TTF_IDISHWND;
			m_ToolTip.hwnd = m_pManager->GetPaintWindow();
			m_ToolTip.uId = (UINT_PTR) m_pManager->GetPaintWindow();
			m_ToolTip.hinst = m_pManager->GetInstance();
			m_ToolTip.lpszText = const_cast<LPTSTR>( (LPCTSTR) sToolTip );
			m_ToolTip.rect = this->GetPos();
			if( m_hwndTooltip == NULL ) {
				m_hwndTooltip = ::CreateWindowEx(0, TOOLTIPS_CLASS, NULL, WS_POPUP | TTS_NOPREFIX | TTS_ALWAYSTIP, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, m_pManager->GetPaintWindow(), NULL, m_pManager->GetInstance(), NULL);
	...
	if( event.Type == UIEVENT_CONTEXTMENU )
	{
		...
		CButtonUI* pButton=(CButtonUI*)(m_pManager->FindControl(_T("menuBtn")));
		if(pButton)
			pButton->Activate();
...
* @retval   void
* 该函数在控件生成函数DialogBuilder::Parse中被调用，控件的属性在xml配置文件
* 中描述。被解析引擎解析为名称和值对应的字符串，该函数通过字符串对比把名称和
* 值存储在控件的数据成员中。
*/
...
* @brief	对全局属性的设置
*
* @param    LPCTSTR pstrList	描述属性的字符串	
* @return	被设置属性的控件
* @retval   CControlUI*
* 有些同类的控件具有相同的属性，可以把这些属性放到全局属性描述字符串中，而不用
* 重复为每个控件对象描述其属性。当动态生成控件对象时通过该函数设置这些全局属性。
*/
...
*
* @param    HDC hDC
* @param    const RECT & rcPaint	被绘制的区域
* @retval   void
* PaintBkColor	绘制背景色
* PaintBkImage	绘制默认背景
* PaintStatusImage	绘制状态背景
* PaintText		绘制文本
* PaintBorder	绘制边框
*/
